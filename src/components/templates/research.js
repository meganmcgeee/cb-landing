...
class Project extends React.Component {
  ...
  render(props) {
    ...
    const gallery = this.props.data.prismicResearch.data.gallery.map(
      (image, index) => (
        <div key={`research_gallery_images_${index}`}>
          <StyledImage>
      ...
      <Layout>
        <Location>
          {({ location }) => {
            return (
              <Helmet>
                <title>{`Caroline Boseley – ${this.props.data.prismicResearch.data.title.text}`}</title>
                <meta
                  name="title"
                  content={`Caroline Boseley – ${this.props.data.prismicResearch.data.title.text}`}
                />
                <meta
                  name="description"
                  content={this.props.data.prismicResearch.data.title.text}
                />
                <meta
                  property="og:url"
                  content={
            ...
            <Row justifyContent="center">
              <Col col={12} sm={12}>
                ...
                >
                  <img
                    src={
                      this.props.data.prismicResearch.data.gallery[0].image.url
                    }
                  />
                </GalleryTrigger>
              </Col>
...
export const query = graphql`
  query SingleResearch($uid: String!) {
