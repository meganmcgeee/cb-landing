...
class Project extends React.Component {
  ...
  render(props) {
    ...
    const projectInformation = this.props.data.prismicEvent.data.project_information.map(
      (text, index) => <p key={index}>{text.project_text}</p>
    );

    console.log(this.props.data.prismicEvent.data.text);
      ...
      <Layout>
        ...
        <Row>
          ...
          <Col col={12} sm={8} lg={8} xl={6}>
            ...
            {this.props.data.prismicEvent.data.video && (
              <Iframe
                dangerouslySetInnerHTML={{
                  __html: this.props.data.prismicEvent.data.video.html,
                }}
              />
            )}
          </Col>
